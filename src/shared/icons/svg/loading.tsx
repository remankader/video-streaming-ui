function LoadingSvg({
  ...
  const svgStyleWhenText = "w-5 h-5";

  return (
    <div className="place-self-center w-full flex justify-center">
      <svg
        aria-hidden="true"
        className={`text-slate-300 text-opacity-70 animate-spin fill-slate-100 ${
          text ? svgStyleWhenText : "w-9 h-9"
        } ${className}`}
        viewBox="0 0 100 101"
        fill="none"
      ...
      <div className={text ? "pl-1.5" : ""}>{text}</div>
    </div>
  );
}
